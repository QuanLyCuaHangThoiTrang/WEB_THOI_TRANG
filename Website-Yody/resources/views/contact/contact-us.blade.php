@php
    ...
    $commonData = [
        'en' => [
            ...
            'message' => 'Message',
            'button' => 'Send Message',
            'description' => 'Having an issue? Contact us now!',
        ],
        'vi' => [
            'name' => 'Tên',
            'email' => 'Email',
            ...
            'description' => 'Bạn đang gặp vấn đề? Liên hệ ngay!',
        ],
    ];
                    ...
                    <div class="relative z-10 max-w-6xl mx-auto flex flex-col lg:flex-row">
                        ...
                        <div class="p-10 rounded-2xl flex-1">
                            <div class="bg-gray-200 p-8 rounded-lg shadow-2xl shadow-gray-800">
                                <h2
                                    class="mb-4 text-3xl sm:text-4xl md:text-5xl lg:text-6xl tracking-tight font-extrabold text-center uppercase text-blue-900">
                                    Yo<span class="text-yellow-500">dy</span>
                                </h2>
                                <p class="text-gray-400 text-center">{{ $selectedData['description'] }}</p>
                                <form class="mt-8 space-y-4 flex-col flex flex-grow-0">
                                    <input type='text' placeholder='{{ $selectedData['name'] }}'
                                        class="w-full rounded-lg py-3 px-4 text-gray-800 text-sm outline-blue-800" />
                                    <input type='email' placeholder='{{ $selectedData['email'] }}'
                                        class="w-full rounded-lg py-3 px-4 text-gray-800 text-sm outline-blue-800" />
                                    <input type='text' placeholder='{{ $selectedData['subject'] }}'
                                        class="w-full rounded-lg py-3 px-4 text-gray-800 text-sm outline-blue-800" />
                                    <textarea placeholder='{{ $selectedData['message'] }}' rows="6"
