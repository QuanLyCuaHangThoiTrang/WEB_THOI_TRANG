...
@endphp
<form action="{{ route('cart.update', ['locale' => $locale]) }}" method="POST" class="updateCart" style="display:inline;">
    @csrf
    @method('PUT')
    @if (Auth::check())
        @foreach ($chiTietGioHang as $index => $chitiet)
            <div id="confirmModal-{{ $index }}"
                class=" fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center backdrop-blur-sm transition-opacity duration-300 ease-in-out">
                <div
            ...
            button.addEventListener('click', function() {
                ...
                if (isNaN(value) || value <= 1) {
                    ...
                    currentModal = document.getElementById(
                        `confirmModal-${index}`); // Lưu modal tương ứng
