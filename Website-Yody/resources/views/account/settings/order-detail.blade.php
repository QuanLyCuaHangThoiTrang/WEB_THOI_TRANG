            ...
            @if($order->chiTietDonHang && $order->chiTietDonHang->isNotEmpty())
                @foreach($order->chiTietDonHang as $index => $chiTiet)
                    @if($chiTiet->chiTietSanPham)
                        <li class="flex px-5 flex-col sm:flex-row justify-between items-start sm:items-center text-base shadow-inner lg:text-lg py-4 border-b border-gray-200 {{ $index % 2 == 0 ? ' bg-zinc-50' : 'bg-white' }}">
