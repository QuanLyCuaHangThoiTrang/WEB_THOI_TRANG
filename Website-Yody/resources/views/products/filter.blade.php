<form class="hidden lg:block">
    ...
    <div class="border-b border-gray-200 py-6">
        <h3 class="-my-3 flow-root">
            <button type="button" class="flex w-full items-center justify-between bg-white py-3 text-sm text-gray-400 hover:text-gray-500" aria-controls="filter-section-0" aria-expanded="true">
    ...
    <div class="flex items-center">
        ...
        <input id="filter-color-{{ $loop->index }}" name="color[]" value="{{ $item->TenMau }}" type="checkbox" class="">
        
        <!-- Hộp màu mà người dùng sẽ nhấp vào -->
        <label for="filter-color-{{ $loop->index }}" 
               class="ml-3 text-sm text-gray-600 flex items-center cursor-pointer"
               onclick="toggleColor(this)">
            <span class="inline-block h-6 w-6 rounded-full mr-2 border border-gray-300" style="background-color: {{ $item->TenMau }};"></span>
            {{ $item->TenMau }}
        </label>
    </div>    
    @endforeach
    ...
    <div class="border-b border-gray-200 py-6">
        <h3 class="-my-3 flow-root">
            <button type="button" class="flex w-full items-center justify-between bg-white py-3 text-sm text-gray-400 hover:text-gray-500" aria-controls="filter-section-2" aria-expanded="true">
